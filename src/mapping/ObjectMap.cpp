...
void ObjectMap::update_objects(RgbdFramePtr frame)
{
  //- Initialization
  if(v_objects.size() == 0)
  {
...
void ObjectMap::writeObjectsToDisk(std::string file_name)
{
  // semantic map
  std::string obj_file_name = file_name+"_object.txt";
  std::ofstream objectfile;
  objectfile.open(obj_file_name, std::ios::out);
  if(objectfile.is_open())
  {
    objectfile << v_objects.size() << "\n";
  }
  objectfile.close();
  for(size_t i=0; i<v_objects.size(); ++i)
  {
    v_objects[i]->writeToFile(obj_file_name);
  }
  std::cout << "Object map wrote to " << obj_file_name << std::endl;

  // keyframes
  std::ofstream kfsfile;
  kfsfile.open(file_name+"_keyframes.txt", std::ios::out);
  if(kfsfile.is_open())
  {
    for(size_t i=0; i<vKFs.size(); ++i)
    {
      Eigen::Matrix3f rot = vKFs[i].topLeftCorner<3,3>();
      Eigen::Vector3f trans = vKFs[i].topRightCorner<3,1>();
  ...
  kfsfile.close();
  std::cout << "KeyFrames wrote to " << file_name << "_keyframes.txt" << std::endl;
}

void ObjectMap::readObjectsFromDisk(std::string file_name)
{
  // object map
  std::string obj_file_name = file_name+"_object.txt";
  std::ifstream objectfile(obj_file_name, std::ios::in);
  int numObj;
  if (objectfile.is_open())
  {
    std::string sNumObj;
  ...
  {
    std::shared_ptr<Object3d> new_object(new Object3d(obj_file_name, start_line));
    v_objects.push_back(std::move(new_object));
    // start_line ++;
  }
  ...
  if(kfsfile.is_open())
  {
    ...
    while(std::getline(kfsfile, line))
    {
      ...
      Eigen::Quaternionf q(qua);
      Eigen::Vector3f t(trans[0], trans[1], trans[2]);
...
// void ObjectMap::update_planes(RgbdFramePtr frame)
// {
